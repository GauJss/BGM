...
##### Input settings
n = 400; p = 1000; s = 50; FDR = 0.1; rho = 0.5
signal = c(2.50, 2.75, 3.00, 3.25, 3.5)
...
set.seed(871)
A = signal[2]
beta[signal_index]= runif(s, A*sqrt(log(p)/n), A*sqrt(log(p)/n)+0.2)
...
ptime <- system.time({
  r4 <- foreach(icount(trials), .combine=cbind) %dopar% {
    ...
    #knockoffs = function(X) create.gaussian(X, rep(0,p), Sigma = V, method='equi')
    k_stat = function(X, Xk, y) stat.glmnet_coefdiff(X, Xk, y, nfolds=5)
    MX_Knock = knockoff.filter(X, y, knockoffs = create.second_order, statistic=k_stat, fdr = FDR, offset = 1)
...
write.csv(mydata, file = 'HD_linear_rho05_final.csv')



mydata = read.csv('HD_linear_rho05_final.csv')[-1]
...
ggsave("HD_linear_rho05_final.pdf", arrangeGrob(p_fdr, p_power, ncol=2), width = 18, height = 8)
