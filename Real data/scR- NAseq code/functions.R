...
AdaFDR = function(x, y, family = "binomial", off = 1, reduce = 1, lambda_min = 0,
  ...
  for(k in 1:len_kappa){
    ...
    # threshold value
    cutoff[k] <- min(cutoff_set) 
    # the selected variable set given kappa
    selected_index[[k]] <- which( M_pgm_k > cutoff[k])
 ...
 cutoff_max=cutoff[k_max]
 return(list(selected = selected_index, Statistics = M_pgm_max, 
        kappa_max = kappa_max,cutoff_value=cutoff_max))
}
...
DS_glm <- function(X, y, signal_index, num_split = 50,  q = 0.1){
  ...
  for(iter in 1:num_split){
    ...
    #cvfit <- glm.fit(X[sample_index1, ], y[sample_index1], family = "binomial")
    #lambda <- cvfit$lambda.min
    cvfit <- glm(y[sample_index1] ~ X[sample_index1,] - 1, family = 'binomial')
    ...
    beta1 <-as.vector(cvfit$coefficients)
    ...
    if(length(nonzero_index)!=0){
      ...
      beta2[nonzero_index] <- as.vector(glm(y[sample_index2] ~ X[sample_index2,] - 1, family = 'binomial')$coefficients)
